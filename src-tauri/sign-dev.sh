...
# you get the "agent-db wants to use confidential information" prompt every launch.
...
if [ -z "$IDENTITY" ]; then
  ...
  if ! security find-certificate -c "$IDENTITY" "$KEYCHAIN" >/dev/null 2>&1; then
    ...
    PW="agentdbdev"   # non-empty: macOS security rejects empty-password PKCS12 MACs
